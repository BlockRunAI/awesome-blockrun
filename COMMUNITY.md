# BlockRun Community

## Get Help

| Channel | Use For |
|---------|---------|
| [GitHub Issues](https://github.com/blockrunai/awesome-blockrun/issues) | Bug reports, feature requests, technical questions |
| [Telegram](https://t.me/+mroQv4-4hGgzOGUx) | Quick questions, chat, community discussion |

## Stay Updated

- [Twitter/X](https://x.com/BlockRunAI) - Announcements and updates
- [GitHub](https://github.com/blockrunai) - Code and releases

## Contributing

We welcome all contributions! See [CONTRIBUTING.md](./CONTRIBUTING.md) for guidelines.

| Contribution | What You Can Do |
|--------------|-----------------|
| Code | Add features, fix bugs, improve SDKs |
| Docs | Improve documentation, add examples |
| Integrations | Build plugins for agent frameworks |
| Research | Analyze x402 ecosystem data |
| Feedback | Report bugs, request features |

All contributors are recognized in [ACKNOWLEDGMENTS.md](./ACKNOWLEDGMENTS.md).

## Code of Conduct

Be respectful, inclusive, and constructive. We're building the future of AI agent payments together.
