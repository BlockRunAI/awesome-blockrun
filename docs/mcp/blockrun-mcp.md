---
title: BlockRun MCP
description: A Model Context Protocol server that gives Claude Code 74 models, crypto data, voice calls, media generation, and prediction markets with zero API keys.
---

# BlockRun MCP

Give Claude Code access to 98 AI models, 83 crypto data endpoints, voice calls, image/video/music generation, prediction markets (read *and* trade), multi-chain RPC, and a sandbox runtime — all with zero API keys.
