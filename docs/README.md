...
BlockRun is economic infrastructure for the agent era. AI agents discover services, pay in USDC over the [x402 protocol](x402/how-it-works.md), and execute autonomously — **no API keys, no subscriptions, no credit card.** One funded wallet unlocks 64 LLMs, media generation, real-time data, and on-chain execution.
...
Franklin — the AI agent with a wallet. Writes code and spends USDC across 64 models and paid APIs. Free to start.
:::
...
64 LLMs (GPT, Claude, Gemini, DeepSeek, Grok, Kimi, Llama) through one OpenAI-compatible API. Pay per request.
:::
