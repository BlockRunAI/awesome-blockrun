...
- **Grok Imagine** accepts `duration_seconds` from **1 to 15** (default 8) and an optional `image_url` (first frame). `resolution` **is honored and is a billing tier** — `xai/grok-imagine-video` renders `480p`/`720p`, and `xai/grok-imagine-video-1.5` adds `1080p`; omitting it bills and renders `480p`, and anything off-tier returns a `400` before payment. Grok Imagine Video **1.5** is xAI's flagship generation — better physics and prompt adherence, with native synced audio in the same pass. The Seedance-only tuning params (`aspect_ratio`, `generate_audio`, `seed`, RealFace, reference media) are not sent upstream for either Grok SKU.
...
| `xai/grok-imagine-video-1.5` | $0.08 / sec @ 480p (default) · $0.14 / sec @ 720p · $0.25 / sec @ 1080p — official rates, no margin | **8s = $1.12** · **15s = $2.10** (480p: **8s = $0.64**; 1080p: **8s = $2.00**) |
