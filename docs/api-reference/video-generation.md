...
| `reference_videos` | array of `{url, role?}` (1–3) | No | Reference videos for reference-to-video (r2v). **Live** on `bytedance/seedance-2.0`, `bytedance/seedance-2.0-fast` and `bytedance/seedance-2.0-mini`; any other model is a `400` before payment. `url` must be `http(s)` and ≤ 2048 characters. `role`, if sent, must be `"reference"` — it is the only role the upstream honours. Each reference video multiplies the token quote by 1 (so one clip doubles it); see the surcharge note below. Flat `video_url` is rejected with a `400` pointing at this field. |
| `reference_audios` | array of `{url, role?}` (1–3, each ≤15.2s) | No | Reference audios for r2v, same models and same `url` / `role` rules as `reference_videos`. Each reference audio adds 0.3 to the token multiplier. Flat `audio_url` is rejected with a `400` pointing at this field. |
...
| 503 | POST | `reference_videos` / `reference_audios` sent while the operator kill switch is set (`R2V_ENABLED` = `false` / `0` / `no` / `off`). Carries `Retry-After: 300`. No charge. It is checked AFTER the shape and capability checks, so a request that is malformed, or that names a model without reference-media support, gets its `400` rather than being told to retry. Jobs already submitted stay pollable: the poll route never reads the switch. |
